...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            batch, prompt_lengths, total_lengths, entity_vecs = batch
...
def sample_sequence(model, length, context, entity_vecs, device='cpu', eos_token_id=None):
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            outputs = model(batch, labels=batch, entity_vecs=entity_vecs)
            lm_loss = outputs[0]
